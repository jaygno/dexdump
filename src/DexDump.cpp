...
static char* descriptorToDot(const char* str)
{
    ...
    newStr[i] = '\0';
//gyjassert(i == targetLen + arrayDepth * 2);
    ...
    const DecodedInstruction* pDecInsn, char* buf, size_t bufSize)
{
    ...
    case kIndexVaries:
         ...
         */
#if DUMP_TRACE
        outSize = snprintf(buf, bufSize, "<index-varies> // thing@%0*x",  width, index);
#else
        outSize = snprintf(buf, bufSize, "<index-varies>");
#endif
        break;
    case kIndexTypeRef:
#if DUMP_TRACE
        ...
        outSize = snprintf(buf, bufSize, "%s",
                getClassDescriptor(pDexFile, index));
#endif
        break;
    case kIndexStringRef:
#if DUMP_TRACE
        outSize = snprintf(buf, bufSize, "\"%s\" // string@%0*x",   dexStringById(pDexFile, index), width, index);
#else
        outSize = snprintf(buf, bufSize, "\"%s\"",   dexStringById(pDexFile, index));
#endif
        break;
...
#if DUMP_TRACE
                outSize = snprintf(buf, bufSize, "%s.%s:%s // method@%0*x",  methInfo.classDescriptor, methInfo.name,  methInfo.signature, width, index);
#else
                outSize = snprintf(buf, bufSize, "%s.%s:%s",  methInfo.classDescriptor, methInfo.name,  methInfo.signature);
#endif
            } else {
#if DUMP_TRACE
    ...
    case kIndexFieldRef:
        {
            ...
            if (getFieldInfo(pDexFile, index, &fieldInfo)) {
                ...
                outSize = snprintf(buf, bufSize, "<field?> // field@%0*x",
                        width, index);
#else
                outSize = snprintf(buf, bufSize, "<field?>");
#endif
            }
        }
        break;
...
#else
        outSize = snprintf(buf, bufSize, "[%0*x]",  width, index);
#endif
        break;
    case kIndexVtableOffset:
        ...
        outSize = snprintf(buf, bufSize, "[%0*x] // vtable #%0*x",
                width, index, width, index);
#else
        outSize = snprintf(buf, bufSize, "[%0*x]",
                width, index);
#endif
        break;
...
#else
	   	if (instr == kPackedSwitchSignature) {
            printf("packed-switch-data (%d units)",
                insnWidth);
        } else if (instr == kSparseSwitchSignature) {
            printf("sparse-switch-data (%d units)",
                insnWidth);
        } else if (instr == kArrayDataSignature) {
            printf("array-data (%d units)",
                insnWidth);
        } else {
            printf("nop");
        }
...
#else
        printf(" v%d, #int %d", pDecInsn->vA, (s4)pDecInsn->vB);
#endif
        break;
    case kFmt11x:        // op vAA
    ...
    case kFmt21t:        // op vAA, +BBBB
        {
            ...
            printf(" v%d, %04x // %c%04x", pDecInsn->vA,
                ...
                (targ < 0) ? -targ : targ);
#else
            printf(" v%d, %04x", pDecInsn->vA, insnIdx + targ);
#endif
        }
        break;
...
#else
            printf(" v%d, #int %d",  pDecInsn->vA, value);
#endif
        } else {
...
#else
            printf(" v%d, #long %lld",  pDecInsn->vA, value);
#endif
        }
        break;
...
#else
        printf(" v%d, v%d, #int %d",  pDecInsn->vA, pDecInsn->vB, (s4)pDecInsn->vC);
#endif
        break;
    case kFmt22t:        // op vA, vB, +CCCC
        {
            ...
            printf(" v%d, v%d, %04x // %c%04x", pDecInsn->vA, pDecInsn->vB,
                ...
                (targ < 0) ? -targ : targ);
#else
            printf(" v%d, v%d, %04x", pDecInsn->vA, pDecInsn->vB,  insnIdx + targ);
#endif
        }
        break;
...
#else
            printf(" v%d, #float %f",  pDecInsn->vA, conv.f);
#endif
        }
        break;
...
#else
        printf(" v%d, %08x",
            pDecInsn->vA, insnIdx + pDecInsn->vB);
#endif
        break;
...
#if DUMP_TRACE
            printf(" v%d, #double %f // #%016llx",  pDecInsn->vA, conv.d, pDecInsn->vB_wide);
#else 
            printf(" v%d, #double %f",  pDecInsn->vA, conv.d);
#endif
